...
__global__ void dft_nlogd_compute(Complex *res_c, Complex *dft_c, Complex *w_c, int d, int m, const int *r) {
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i >= d * m) return;
    int it = i / m, p = i % m;
    int j = r[it];
...
void dft_nlogd(Complex* a_c, int N, int k, int d, const int *r) {
    ...
    dft_nlogd_compute<<<(d * m + NUM_THREADS - 1) / NUM_THREADS, NUM_THREADS>>> dft_nlogd_compute(d_c, dft_c, w_c, d, m, r);
...
__global__ void dct_load(double *a_re, Complex *dct_c, Complex *dct_shift_c, int d) {
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i >= d) return;
    int j = r[i];
    a_re[i] = (dct_c[j] * dct_shift_c[j]).real();
}

void dct(double *a_re, int N) {
    dct_store<<<(N + NUM_THREADS - 1) / NUM_THREADS, NUM_THREADS>>>(dct_c, a_re, N);
    fft_parallel(dct_c, N, w_c, bit_rev);
    dct_load<<<(d + NUM_THREADS - 1) / NUM_THREADS, NUM_THREADS>>>(a_re, dct_c, dct_shift_c, d);
}

__global__ void dct_nlogd_load(double *a_re, Complex *dct_c, Complex *dct_shift_c, int d) {
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i >= d) return;
    int j = r[i];
...
__global__ void shuffle_real(double *srft_re, double *a_gpu, int *perm_gpu, int *f_gpu) {
    int i = blockIdx.x * blockDim.x + threadIdx.x;
