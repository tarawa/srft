...
double *w_re, *w_im, *temp_re, *temp_im, *w2_re, *w2_im;
int *bit_cnt;
...
#pragma omp single
    {
        temp = new double[N];
        flag = (N < (double)k * k * log2(k));
        bit_cnt = new int[N];
    }
    transpose(a, temp, N, k);
    compute_bitcount(bit_cnt, N);
    if (flag) {
        for (int i = 0; i < N; i += k) fwht_parallel(temp + i, k);
    } else {
#pragma omp for nowait
...
void dft_nlogd(double* a_re, double *a_im, int N, int k, int d, const int *r) {
    double *temp_re, *temp_im, *w_re, *w_im;
    int m = N / k, *bit_rev;
    bool flag;
#pragma omp single
    { ;
        temp_re = new double[N];
        temp_im = new double[N];
        w_re = new double[k + 1];
        w_im = new double[k + 1];
        bit_rev = new int[k];
        flag = (N < (double)k * k * log2(k));
...
#pragma omp barrier
    if (flag) {
        for (int i = 0; i < N; i += k) fft_parallel(temp_re + i, temp_im + i, k, w_re, w_im, bit_rev);
    } else {
#pragma omp for nowait
...
#pragma omp single
    {
        w2_re = new double[N + 1];
        w2_im = new double[N + 1];
    }
    compute_w(w2_re, w2_im, N);
    for (int i = 0; i < d; ++i) {
        int j = r[i];
        int y = j % k;
        ...
        for (int p = 0; p < m; ++p) {
            ...
            double z_re = w2_re[x], z_im = w2_im[x];
